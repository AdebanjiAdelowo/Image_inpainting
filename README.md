...
One iteration of `pdr_inpaint` (`tvinpaint/solver.py`), with $\bar u, \bar p$ the Douglas-Rachford
auxiliary variables:

```mermaid
flowchart TD
    S["state ū, p̄"] --> B["b = ū + στ div p̄"]
    B --> GS["primal step: (λI − μΔ)u = b, μ = (στ)²<br/>inexact, n_sweeps red-black Gauss-Seidel sweeps<br/>(symmetric by default; forward with legacy=True)"]
    GS --> P["dual step: p = p̄ + στ ∇u"]
    GS --> RU["reflect through data set<br/>u_test = P_C(2u − ū): u = f on known pixels"]
    P --> RP["reflect through dual ball<br/>p_test = P_ball(2p − p̄), radius α/τ"]
    RU --> UB["ū ← ū + u_test − u"]
    RP --> PB["p̄ ← p̄ + p_test − p"]
    UB --> S
    PB --> S
    RU -.->|"returned as res.u"| OUT["feasible solution u_test"]
    GS -.->|"returned as res.u_dr"| RAW["raw iterate u"]
```
...
![Hole PSNR against maximum distance to a known pixel for each image and method](benchmark/figures/fig_psnr_vs_hole_size.png)

*Hole PSNR against hole size at a fixed 16.2 % damaged area, per image (filled markers: synthetic
masks, mean and sd over 3 seeds; hollow: the canonical mask). In the benchmark figures "TV, shipped"
is the historical notebook algorithm at 200 iterations (the "TV historical" column above) and
"TV, converged" is the verified solver at 1,500 iterations. Generated by
`benchmark/make_figures.py`.*

![Crops around one 64x64 hole for each image: ground truth, damaged input, and four reconstructions](benchmark/figures/fig_comparison_blocks64.png)

*One 64 px hole per image (seed 0). At 200 iterations the historical solver leaves a flat grey patch
in the hole; converged TV continues the phantom's edge into the hole but fills textured `grass`
with a flat value.*
...
* **Convergence and budget.** The default 200 iterations are not converged for large holes. Compared
  ...
  ![Relative distance to the 1500-iteration result and hole-PSNR deficit against iteration count, by mask](benchmark/figures/fig_convergence.png)

  *Convergence by hole size (symmetric sweep, all images). The dashed line marks the default 200
  iterations; the largest holes are still several dB short of the 1,500-iteration result there.*
